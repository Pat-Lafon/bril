    ...
    pub fn insert_block_between(
        ...
        let new_block = BasicBlock {
            ...
            code: vec![Instruction::Effect {
                ...
                args: Vec::new(),
                funcs: Vec::new(),
                labels: vec![previous_block.label.clone()],
            }],
            ...
            match pred_block.successor {
                ...
                Successor::Jump(_) => {
                    ...
                    pred_block.code.push(Instruction::Effect {
                        ...
                        args: Vec::new(),
                        funcs: Vec::new(),
                        labels: vec![new_block_label.clone()],
                    });
                }
                Successor::Conditional {
                    ...
                    if block == true_branch {
                        ...
                        let mut args = final_instr.get_labels().unwrap();
                        args[0] = new_block_label.clone();
                        final_instr.set_labels(args);
    ...
    for i in code.into_iter() {
        match i {
            ...
            Code::Instruction(Instruction::Effect {
                ...
                debug_assert!(labels.is_empty());
                debug_assert!(args.is_empty() || args.len() == 1);
                debug_assert!(funcs.is_empty());
                current_code.push(Code::Instruction(Instruction::Effect {
                    op: EffectOps::Return,
                    labels,
            ...
            Code::Instruction(Instruction::Effect {
                ...
                let t_branch = get_number(labels[0].clone());
                let f_branch = get_number(labels[1].clone());
